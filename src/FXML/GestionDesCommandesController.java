...
public class GestionDesCommandesController {
    ...
    @FXML
    private TableView<ObservableList> tableGestionDesCommandes;

    
    @FXML
    void HomeActionReturn1(ActionEvent event) {
        try{
    ...
    public void initialize(){
        try {
            String sql =  "select * from gestiondecommandes where 1" ;
            ...
            for(int i=0 ; i<rs.getMetaData().getColumnCount(); i++){
                ...
                tableGestionDesCommandes.getColumns().addAll(col); 
            }
            while(rs.next()){
                //Iterate Row
            ...
            tableGestionDesCommandes.setItems(data);
    ...
    void AjouterOnAction(ActionEvent event) {
        try {
            String sql = "INSERT INTO gestiondecommandes (numc, datec, montantc, fournass, validite) VALUES ('"+nomc.getText()+"','"+datec.getText()+"','"+montantc.getText()+"','"+fournass.getText()+"','"+val.getText()+"')";
            Statement smt = PhManager.con.createStatement() ;
    ...
    void SupprimerOnAction(ActionEvent event) {
        ObservableList<ObservableList> dataa = (ObservableList<ObservableList>) tableGestionDesCommandes.getSelectionModel().getSelectedItem();
        try {
            String sql =  "DELETE FROM gestiondecommandes WHERE numc = '"+(String)(Object)dataa.get(0)+"'" ;
            Statement smt = PhManager.con.createStatement();
            smt.executeUpdate((sql)) ;
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Succès");
            alert.setHeaderText(null);
            alert.setContentText("Commande "+(String)(Object)dataa.get(0)+" supprimée");
    ...
    void ModifierOnAction(ActionEvent event) {
        ObservableList<ObservableList> dataa = tableGestionDesCommandes.getSelectionModel().getSelectedItem();
        selectedname =(String)(Object)dataa.get(0);
        try {
            Stage logp = new Stage();
            Parent root;
            root = FXMLLoader.load(getClass().getResource("ModifierGestionDesCommandesInterface.fxml"));
            Scene scene = new Scene(root);
            logp.setScene(scene);
            logp.show();
    ...
    void RechercherOnAction(ActionEvent event) {
        ...
        ObservableList<ObservableList> dataa =data.stream().filter(cas -> cas.get(0).toString().contains(vaal)).collect(Collector.of(
            ...
            //FINALLY ADDED TO TableView
        tableGestionDesCommandes.setItems(dataa);
    }
}
